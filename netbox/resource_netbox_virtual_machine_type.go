...
func resourceNetboxVirtualMachineTypeCreate(d *schema.ResourceData, m interface{}) error {
	...
	if platformID, ok := d.GetOk("default_platform_id"); ok {
		data.DefaultPlatform = int64ToPtr(int64(platformID.(int)))
	}

	if vcpus, ok := d.GetOk("default_vcpus"); ok {
		data.DefaultVcpus = float64ToPtr(vcpus.(float64))
	}

	if memory, ok := d.GetOk("default_memory_mb"); ok {
		data.DefaultMemory = int64ToPtr(int64(memory.(int)))
	}
...
func resourceNetboxVirtualMachineTypeRead(d *schema.ResourceData, m interface{}) error {
	...
	if vmType.DefaultPlatform != nil {
		d.Set("default_platform_id", vmType.DefaultPlatform.ID)
	} else {
		d.Set("default_platform_id", nil)
	}

	d.Set("default_vcpus", vmType.DefaultVcpus)
	d.Set("default_memory_mb", vmType.DefaultMemory)
...
func resourceNetboxVirtualMachineTypeUpdate(d *schema.ResourceData, m interface{}) error {
	...
	// These are nullable, so send the zero value rather than omitting it when
	// the attribute is removed; otherwise the partial update cannot clear them.
	if platformID := d.Get("default_platform_id").(int); platformID != 0 {
		data.DefaultPlatform = int64ToPtr(int64(platformID))
	}

	if vcpus := d.Get("default_vcpus").(float64); vcpus != 0 {
		data.DefaultVcpus = float64ToPtr(vcpus)
	}

	if memory := d.Get("default_memory_mb").(int); memory != 0 {
		data.DefaultMemory = int64ToPtr(int64(memory))
	}
