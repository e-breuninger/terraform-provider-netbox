...
func dataSourceNetboxRackGroup() *schema.Resource {
	return &schema.Resource{
		Read:        dataSourceNetboxRackGroupRead,
...
func dataSourceNetboxRackGroupRead(d *schema.ResourceData, m interface{}) error {
	api := m.(*providerState)
	...
	if *res.GetPayload().Count == int64(0) {
		return errors.New("no rack group found matching filter")
	}
