...
func dataSourceNetboxDeviceRearPortsRead(d *schema.ResourceData, m interface{}) error {
	...
	var allRearPorts []*models.RearPort
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("failed to fetch rear ports at offset %d: %w", currentOffset, err)
		}

		payload := res.GetPayload()
		allRearPorts = append(allRearPorts, payload.Results...)

		if len(payload.Results) == 0 {
			break
		}

		if !paginationHelper.ShouldContinuePaging(int64(len(allRearPorts)), payload.Next) {
			break
		}
	...
	if nameRegex, ok := d.GetOk("name_regex"); ok {
		...
		for _, rearPort := range allRearPorts {
			if r.MatchString(*rearPort.Name) {
				filteredRearPorts = append(filteredRearPorts, rearPort)
			}
		}
	} else {
		filteredRearPorts = allRearPorts
	}

	// Apply user limit to filtered results
	if userLimit > 0 && int64(len(filteredRearPorts)) > userLimit {
		filteredRearPorts = filteredRearPorts[:userLimit]
	}

	if len(filteredRearPorts) == 0 {
